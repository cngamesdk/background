<template>
  <el-popover placement="bottom-start" :width="500" trigger="click" @hide="hideComponent">
    <template #reference>
      <el-button>指标选择({{ selectedNum }})</el-button>
    </template>

    <div class="item-container" v-for="(group, index) in localIndicators" :key="index">
      <div class="title">
        <el-checkbox
...
const localIndicators = reactive([])
...
const hideComponent = () => {
  let selectedValues = []
  localIndicators.forEach(function (item) {
...
const handleGroupAllChange = (val, index) => {
  const indexValue = localIndicators[index]
  localIndicators[index].selected = val ? indexValue.childs.map(opt => opt.key) : []
  localIndicators[index].indeterminate = (indexValue.selected.length > 0 && indexValue.selected.length < indexValue.childs.length)
...
const handleGroupChange = (val, index) => {
  const indexValue = localIndicators[index]
  localIndicators[index].selected = val ? val : []
  localIndicators[index].indeterminate = (indexValue.selected.length > 0 && indexValue.selected.length < indexValue.childs.length)
  localIndicators[index].allChecked = (indexValue.selected.length > 0 && indexValue.selected.length >= indexValue.childs.length)
