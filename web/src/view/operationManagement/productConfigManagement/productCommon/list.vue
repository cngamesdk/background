    ...
    <div class="gva-search-box">
      <el-form ref="searchForm" :inline="true" :model="searchInfo">
        <el-form-item label="配置ID/配置名称">
          <el-input v-model="searchInfo.config_name" placeholder="配置ID/配置名称" />
        </el-form-item>
        <el-form-item>
    ...
    >
      <el-form ref="configForm" :model="configInfo" :rules="rules" label-width="auto">
        ...
        <el-form-item label="配置名称" prop="config_name">
          <el-input v-model="configInfo.config_name" placeholder="请输入配置名称"/>
        </el-form-item>
        <el-form-item label="发货地址" prop="shipping_address">
          <el-input v-model="configInfo.shipping_address" placeholder="请输入发货地址"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
import { productCommonConfigurationList, productCommonConfigurationAdd, productCommonConfigurationModify } from '@/api/operationManagement'
...
defineOptions({
  name: 'ProductCommonConfigurationList'
})
...
const getTableData = async () => {
  const table = await productCommonConfigurationList({
    page: page.value,
  ...
  configForm.value.validate(async (valid) => {
    if (valid) {
      if (configDialog.value.add) {
        const res = await productCommonConfigurationAdd(configInfo.value)
        if (res.code === 0) {
        ...
        const res = await productCommonConfigurationModify(configInfo.value)
        if (res.code === 0) {
