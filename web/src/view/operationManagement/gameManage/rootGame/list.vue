  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="searchForm" :inline="true" :model="searchInfo">
        <el-form-item label="根游戏ID/根游戏名称">
          <el-input v-model="searchInfo.game_name" placeholder="根游戏ID/根游戏名称" />
        </el-form-item>
        <el-form-item>
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        <el-button type="primary" icon="plus" @click="openAddConfigDialog">新增根游戏</el-button>
      </div>
    ...
    >
      <el-form ref="configForm" :model="configInfo" :rules="rules" label-width="auto">
        ...
        <el-form-item label="根游戏名称" prop="game_name">
          <el-input v-model="configInfo.game_name" placeholder="请输入根游戏名称"/>
        </el-form-item>
        <el-form-item label="合同游戏名称" prop="contract_name">
          <el-input v-model="configInfo.contract_name" placeholder="请输入合同游戏名称"/>
        </el-form-item>
        <el-form-item label="与研发分成比例" prop="profit_sharing_ratio">
          <el-input-number :min="0" :max="100" v-model="configInfo.profit_sharing_ratio" placeholder="请输入与研发分成比例"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
import { rootGameList, rootGameAdd, rootGameModify } from '@/api/operationManagement'
...
const getTableData = async () => {
  const table = await rootGameList({
    page: page.value,
...
const rules = ref({
  ...
  game_name: [
    { required: true, message: '请输入根游戏名称', trigger: 'blur' },
  ],
  contract_name: [
    { required: true, message: '请输入游戏合同名称', trigger: 'blur' },
  ],
  profit_sharing_ratio: [
    { required: true, message: '请输入研发分成比例', trigger: 'blur' },
    { pattern: /^[1-9]\d*$/, message: '请输入研发分成比例', trigger: 'blur' }
  ]
})
  ...
  configForm.value.validate(async (valid) => {
    if (valid) {
      if (configDialog.value.add) {
        const res = await rootGameAdd(configInfo.value)
        if (res.code === 0) {
        ...
        const res = await rootGameModify(configInfo.value)
        if (res.code === 0) {
