  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="searchForm" :inline="true" :model="searchInfo">
        <el-form-item label="主游戏ID/主游戏名称">
          <el-input v-model="searchInfo.game_name" placeholder="主游戏ID/主游戏名称" />
        </el-form-item>
        <el-form-item>
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        <el-button type="primary" icon="plus" @click="openAddConfigDialog">新增主游戏</el-button>
      </div>
    ...
    >
      <el-form ref="configForm" :model="configInfo" :rules="rules" label-width="auto">
        ...
        <el-form-item label="所属根游戏" prop="root_game_id">
          <el-select v-model="configInfo.root_game_id"
                     placeholder="请选择所属根游戏"
                     filterable
                     remote
                     reserve-keyword
                     :remote-method="getRootGames"
                     :loading="loading"
                     style="width: 240px">
            <el-option
                v-for="item in rootGames"
                :key="item.key"
                :label="item.value"
                :value="item.key"
            />
        ...
        <el-form-item label="主游戏名称" prop="game_name">
          <el-input v-model="configInfo.game_name" placeholder="请输入主游戏名称"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
import { mainGameList, mainGameAdd, mainGameModify } from '@/api/operationManagement'
import { searchPlatform, searchRootGame } from '@/api/systemManagement'
...
const getRootGames = async (query) => {
  const table = await searchRootGame({keyword: query})
  if (table.code === 0) {
    rootGames.value = table.data
  }
}

// 查询
const getTableData = async () => {
  const table = await mainGameList({
    page: page.value,
...
const rules = ref({
  ...
  root_game_id: [
    { required: true, message: '请选择所属根游戏', trigger: 'blur' },
    { pattern: /^[1-9]\d*$/, message: '请选择所属根游戏', trigger: 'blur' }
  ],
  game_name: [
    { required: true, message: '请输入主游戏名称', trigger: 'blur' },
  ]
})
  ...
  configForm.value.validate(async (valid) => {
    if (valid) {
      if (configDialog.value.add) {
        const res = await mainGameAdd(configInfo.value)
        if (res.code === 0) {
        ...
        const res = await mainGameModify(configInfo.value)
        if (res.code === 0) {
