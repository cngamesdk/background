  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="searchForm" :inline="true" :model="searchInfo">
        <el-form-item label="渠道组ID/渠道组名称">
          <el-input v-model="searchInfo.channel_group_name" placeholder="渠道组ID/渠道组名称" />
        </el-form-item>
        <el-form-item>
    ...
    >
      <el-form ref="configForm" :model="configInfo" :rules="rules" label-width="auto">
        ...
        <el-form-item label="媒体" prop="advertising_media_id">
          <el-select v-model="configInfo.advertising_media_id" placeholder="请选择所属媒体" style="width: 240px">
            <el-option
                v-for="item in advertisingMedias"
                :key="item.key"
                :label="item.value"
                :value="item.key"
            />
        ...
        <el-form-item label="渠道组名称" prop="channel_group_name">
          <el-input v-model="configInfo.channel_group_name" placeholder="请输入渠道组名称"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
import { channelGroupList, channelGroupAdd, channelGroupModify } from '@/api/advertising'
...
const configInfo = ref({
  id: 0,
  platform_id: 0,
  advertising_media_id: 0,
  channel_group_name: '',
})

const platforms = ref([])
const advertisingMedias = ref([])
...
const getAdvertisingMedias = async () => {
  const table = await searchMedia()
  if (table.code === 0) {
    advertisingMedias.value = table.data
  }
}

// 查询
const getTableData = async () => {
  const table = await channelGroupList({
    page: page.value,
...
const rules = ref({
  ...
  advertising_media_id: [
    ...
    { pattern: /^[1-9]\d*$/, message: '请选择媒体', trigger: 'blur' }
  ],
  channel_group_name: [
  ...
  configForm.value.validate(async (valid) => {
    if (valid) {
      if (configDialog.value.add) {
        const res = await channelGroupAdd(configInfo.value)
        if (res.code === 0) {
        ...
        const res = await channelGroupModify(configInfo.value)
        if (res.code === 0) {
